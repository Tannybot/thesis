...
const COLORS = ['#8b5cf6', '#22d3ee', '#f59e0b', '#f43f5e', '#6366f1', '#ec4899'];

const tooltipStyle = {
  background: 'rgba(19, 17, 43, 0.95)',
  border: '1px solid rgba(139, 92, 246, 0.2)',
  borderRadius: '14px',
  color: '#e2e8f0',
  padding: '12px 16px',
  boxShadow: '0 12px 40px rgba(0, 0, 0, 0.5), 0 0 20px rgba(139, 92, 246, 0.1)',
};
...
export default function DashboardPage() {
  ...
  const statCards = [
    { label: 'Total Animals', value: stats.total_animals, icon: PawPrint, gradient: 'linear-gradient(135deg, #8b5cf6, #6366f1)', shadowColor: 'rgba(139, 92, 246, 0.3)' },
        ...
        {statCards.map((card, i) => (
          <div key={i} className="glass-card stat-card animate-in" style={{ animationDelay: `${i * 0.08}s` }}>
            <div className="flex items-center justify-between mb-4">
              ...
              <ArrowUpRight size={18} style={{ color: 'rgba(167, 139, 250, 0.4)' }} />
            </div>
            <p className="text-3xl font-black text-white leading-none">{card.value}</p>
            <p className="text-[13px] mt-2 font-medium" style={{ color: 'rgba(167, 139, 250, 0.6)' }}>{card.label}</p>
          </div>
        ))}
      </div>
          ...
          {stats.growth_stage_breakdown.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={stats.growth_stage_breakdown} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="stage" tick={{ fill: 'rgba(167, 139, 250, 0.5)', fontSize: 12 }} axisLine={false} tickLine={false} tickMargin={12} />
                <YAxis tick={{ fill: 'rgba(167, 139, 250, 0.5)', fontSize: 12 }} axisLine={false} tickLine={false} tickMargin={12} />
                <Tooltip cursor={{ fill: 'rgba(139, 92, 246, 0.06)' }} contentStyle={tooltipStyle} />
          ...
          <div className="glass-card overflow-x-auto min-w-0 lg:col-span-2">
            ...
            <div className="grid grid-cols-1 min-[480px]:grid-cols-2 md:grid-cols-4 gap-[16px]">
              {[
                ...
                { label: 'Sold / Marketed', value: stats.sold_animals, color: '#818cf8' },
                ...
                activities.slice(0, 8).map((a, i) => (
                  ...
                  >
                    <div
                      ...
                      style={{
                        background: a.type === 'registration' ? '#8b5cf6' :
                          a.type === 'health' ? '#f59e0b' :
                          a.type === 'movement' ? '#22d3ee' : '#6366f1',
                        boxShadow: `0 0 8px ${
                          a.type === 'registration' ? 'rgba(139, 92, 246, 0.5)' :
                          a.type === 'health' ? 'rgba(245, 158, 11, 0.5)' :
                          a.type === 'movement' ? 'rgba(34, 211, 238, 0.5)' : 'rgba(99, 102, 241, 0.5)'
                        }`,
                      }}
                    />
                    <div className="min-w-0">
                      <p className="text-[14px] font-medium text-white/90 truncate">{a.title}</p>
                      <p className="text-[13px] truncate mt-1 leading-relaxed" style={{ color: 'rgba(167, 139, 250, 0.5)' }}>{a.description}</p>
                    </div>
                  </div>
                ))
              ) : (
        ...
        {healthData?.monthly_records?.length > 0 ? (
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={healthData.monthly_records} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(139, 92, 246, 0.08)" />
              <XAxis dataKey="month" tick={{ fill: 'rgba(167, 139, 250, 0.5)', fontSize: 12 }} axisLine={false} tickLine={false} tickMargin={12} />
              <YAxis tick={{ fill: 'rgba(167, 139, 250, 0.5)', fontSize: 12 }} axisLine={false} tickLine={false} tickMargin={12} />
              <Tooltip contentStyle={tooltipStyle} />
              <Area
                type="monotone"
