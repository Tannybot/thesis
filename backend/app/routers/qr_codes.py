...
def _qr_needs_plain_text_refresh(qr: QRCode | None) -> bool:
    if not qr or not qr.qr_data:
        return True
    return not qr.qr_data.startswith(("http://", "https://")) or "/animals/qr/" not in qr.qr_data
        ...
        "treatments": [
            {
                ...
                "treatment_date": _serialize_date(treatment.treatment_date),
                "next_treatment_date": _serialize_date(treatment.next_treatment_date),
                "administered_by": treatment.administered_by,
            }
            for treatment in sorted(animal.treatments, key=lambda item: item.treatment_date or date.min, reverse=True)
        ],
        "vaccinations": [
            {
                "vaccine_name": vaccination.vaccine_name,
                "vaccination_date": _serialize_date(vaccination.vaccination_date),
                "next_due_date": _serialize_date(vaccination.next_due_date),
                "administered_by": vaccination.administered_by,
            }
            for vaccination in sorted(animal.vaccinations, key=lambda item: item.vaccination_date or date.min, reverse=True)
        ],
        "movements": [
            {
...
def get_qr_code(
    ...
    if not qr or _qr_needs_plain_text_refresh(qr):
...
def public_trace_animal(qr_token: str, db: Session = Depends(get_db)):
    """Resolve a public QR token to animal traceability details."""
